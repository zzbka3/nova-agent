/*
 * @Author: hewenquan
 * @Date: 2025-07-21 17:20:48
 * @LastEditTime: 2025-10-23 14:37:31
 * @LastEditors: hewenquan
 * @FilePath: /metis-front/src/views/flow/registerFlowNode/registerLargeModel.js
 * @Description: 大模型
 */
import Vue from 'vue';
...
export default function registerConnect(lf) {
  lf.register('LLM', ({ HtmlNode, HtmlNodeModel }) => {
    class largeModelNode extends HtmlNode {
      setHtml(rootEl) {
    ...
    class largeModelNodeModel extends HtmlNodeModel {
      initNodeData(data) {
        const { nodeName = '', width = 400 } = this.properties || {};
        let name = getNodeInitNames(nodeName, 'LLM', lf, '大模型');
