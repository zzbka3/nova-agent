...
export default function registerConnect(lf) {
  lf.register('TEXT_PROCESSOR', ({ HtmlNode, HtmlNodeModel }) => {
    class textProcessorNode extends HtmlNode {
      setHtml(rootEl) {
    ...
    class textProcessorNodeModel extends HtmlNodeModel {
      initNodeData(data) {
        if (data.text) {
          data.text.editable = false;
        }
        const { nodeName = '', width = 400 } = this.properties || {};
        let name = getNodeInitNames(nodeName, 'TEXT_PROCESSOR', lf, '文本处理');
    ...
    return {
      view: textProcessorNode,
      model: textProcessorNodeModel
    };
  });
}