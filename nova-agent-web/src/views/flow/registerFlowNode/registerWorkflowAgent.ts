...
import workFlowAgent from './components/workFlowAgent.vue'
...
export default function registerWorkflowAgent(lf: any) {
  lf.register('WORKFLOW_AGENT', ({ HtmlNode, HtmlNodeModel }: any) => {
    class AgentNode extends HtmlNode {
      setHtml(rootEl: HTMLElement) {
        const model = this.props.model
        const el = document.createElement('div')
        rootEl.innerHTML = ''
        rootEl.appendChild(el)
        const app = createApp(workFlowAgent, { name: model.properties?.name, model, lf })
        app.mount(el)
        setTimeout(() => {
    ...
    class AgentNodeModel extends HtmlNodeModel {
      initNodeData(data: any) {
        if (data.text) data.text.editable = false
        const { nodeName = '' } = this.properties || {}
        let name = getNodeInitNames(nodeName, 'WORKFLOW_AGENT', lf, '工作流')
    ...
    return { view: AgentNode, model: AgentNodeModel }
  })
}
