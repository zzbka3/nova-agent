<template>
  <div :class="['node-container', { 'node-selected': isSelected }]" ref="nodeRef">
    ...
    <queryInfo v-show="allShow" info-title="条件" :info-data="inputVars" :arr-args="arrArgs" @updateNodeAttributes="updateNodeAttributes" />
...
import { ifIconSvg } from '../../common/icons'
import { getAllArgs } from '../../getArgs'
const props = defineProps<{ model: any; lf: any }>()
const bus: any = inject('$bus')
const isSelected = ref(false); const allShow = ref(true); const arrArgs = ref<any[]>([]); const inputVars = ref<any[]>([]); const outputList = ref<any[]>([]); const nodeRef = ref()
const icon = ifIconSvg; const getNodeId = computed(() => props.model?.id); const getNodeName = computed(() => props.model?.properties?.nodeName)
onMounted(() => {
...
function updateNodeAttributes(e?: boolean) { setTimeout(() => { const el = document.querySelector('.node-container') as HTMLElement; const m = props.lf.getNodeModelById(getNodeId.value); if (el?.clientHeight > 0) m?.setCustomAttributes({ currentHeight: el.clientHeight, expand: e }) }, 0) }
</script>
<style lang="less" scoped>@import url('../../customCss/index.less'); .node-container { width:100%; background:#fff; border-radius:12px; padding:12px; text-align:left; min-width:400px; }</style>
