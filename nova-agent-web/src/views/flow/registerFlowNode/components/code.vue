<template>
    ...
    >
        ...
        <nodeTitle
            ...
            :node-icon="require('../../image/code.png')"
...
import validateCodeNode from '../../validateUtils/validateCodeNode';
...
export default {
    ...
    methods: {
        ...
        updateNodeAttributes(expand) {
            this.$nextTick(() => {
                const clientHeight = this.$refs[`code_${this.getId}`]?.clientHeight;
        ...
        validateFlowChild() {
            const { validateStatus, errResult } = validateCodeNode({
                model: this.model,
                lf: this.lf
            });
