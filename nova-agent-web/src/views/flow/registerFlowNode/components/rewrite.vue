<!--
 * @Author: hewenquan
 * @Date: 2025-08-14 10:53:32
 * @LastEditTime: 2025-10-24 16:49:17
 * @LastEditors: hewenquan
 * @FilePath: /metis-front/src/views/flow/registerFlowNode/components/rewrite.vue
 * @Description: query多轮改写
-->
<template>
    <div
        :class="[
    ...
    >
        ...
        <nodeTitle
            ...
            :node-icon="require('../../image/rewrite.png')"
...
import validateRewriteNode from '../../validateUtils/validateRewriteNode';
...
export default {
    ...
    methods: {
        initBus() {
            ...
            this.bus.$on('validateConfigById', ({ nodeId }) => {
                if (nodeId === this.getId) {
                    ...
                    const data = validateRewriteNode({
                        model: this.model,
                        lf: this.lf
                    });
        ...
        validateFlowChild() {
            const { validateStatus, errResult } = validateRewriteNode({
                model: this.model,
                lf: this.lf
            });
