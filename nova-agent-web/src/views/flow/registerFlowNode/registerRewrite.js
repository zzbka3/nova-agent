...
export default function registerConnect(lf) {
  lf.register('REWRITE', ({ HtmlNode, HtmlNodeModel }) => {
    class rewriteNode extends HtmlNode {
      setHtml(rootEl) {
    ...
    class rewriteModel extends HtmlNodeModel {
      initNodeData(data) {
        if (data.text) {
          data.text.editable = false;
        }
        const { nodeName = '', width = 400 } = this.properties || {};
        let name = getNodeInitNames(nodeName, 'REWRITE', lf, 'query 多轮改写');
