<template>
  <div class="container">
    ...
    <p style="color: #84868c; font-size: 12px; margin-top: 12px;">引用另一个工作流 Agent，将上游输出作为该 Agent 的输入执行。</p>
  </div>
</template>
...
function init() {
  ...
  inputVars.value = pd.value.inputVars?.length ? [...pd.value.inputVars] : []
}
...
function updateLf() {
  const { id } = props.clickNode
  props.lf.getNodeModelById(id)?.setProperties({ inputVars: [...inputVars.value] })
}
